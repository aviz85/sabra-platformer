// Renders src/parallax/jerusalem.js (sky + layers + fog) in headless Chrome at 3x and screenshots to shots/.
// Usage: node tools/preview-parallax-jerusalem.mjs   → shots/parallax-jerusalem-cam{0,400}-t{0,2}.png (+ cam600 and a late-dusk cam1200 shot)
// Exits 1 on any console error/warning, page error, or failed request.
...
<script type="module">
  import px from '/src/parallax/jerusalem.js';
  const W = 320, H = 180;
...
const pagePath = path.join('shots', '_preview-jerusalem.html');
...
try {
  ...
  for (const [camX, camY, t] of [[0, 0, 0], [0, 0, 2], [400, 0, 0], [400, 0, 2], [600, 0, 5], [1200, 30, 60]]) {
    await page.evaluate(([cx, cy, tt]) => window.render(cx, cy, tt), [camX, camY, t]);
    const name = `shots/parallax-jerusalem-cam${camX}${camY ? `-y${camY}` : ''}-t${t}.png`;
