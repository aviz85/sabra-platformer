// Renders src/parallax/telaviv.js (sky + layers + fog) in headless Chrome at 3x and screenshots to shots/.
// Usage: node tools/preview-parallax-telaviv.mjs   → shots/parallax-telaviv-cam{0,400}-t{0,2}.png (exit 1 on console errors)
...
<script type="module">
  import px from '/src/parallax/telaviv.js';
  const W = 320, H = 180;
...
const pagePath = path.join('shots', '_preview-telaviv.html');
...
try {
  ...
  for (const [camX, camY, t] of [[0, 0, 0], [0, 0, 2], [400, 0, 0], [400, 0, 2], [1200, 30, 1]]) {
    ...
    const name = `shots/parallax-telaviv-cam${camX}${camY ? `-y${camY}` : ''}-t${t}.png`;
